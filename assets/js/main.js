(function($) {
    ...
    /*-------------------------------------
     ...
     --------------------------------------*/
    $('nav#dropdown').meanmenu({ siteLogo: "<a href='/' class='logo-mobile-menu'><img src='/assets/images/DONAVA-LOGO.png' class='img img-responsive' style='height: 50px;margin-left: 10px;' /></a>" });
  ...
  ***************************************/
    ...
    if (contactForm.length) {
        contactForm.validator().on('submit', function(e) {
            ...
            if (e.isDefaultPrevented()) {
                $target.html("<div class='alert alert-success'><p>Hãy điền các trường bắt buộc (*).</p></div>");
            } else {

                $.ajax({
                    url: "/frontend/Contact/contact_form",
                    type: "POST",
                    ...
                    success: function(text) {
                        if (text == 1 || text == '1') {
                            $this[0].reset();
                           // alert(text);
                            $target.html("<div class='alert alert-success'><p>Thông tin liên hệ đã được gửi.</p></div>");
                        } else {
    ...
    if (requestForm.length) {
        requestForm.validator().on('submit', function(e) {
            ...
            if (e.isDefaultPrevented()) {
                $target.html("<div class='alert alert-success'><p>Hãy điền tất cả các ô.</p></div>");
            } else {
                ...
                $.ajax({
                    ...
                    success: function(text) {
                        if (text == 1 || text == '1') {
                            $this[0].reset();
                            $target.html("<div class='alert alert-success'><p>Thông tin đã được gửi thành công.</p></div>");
                        } else {
...
//alert(margin);
	...
	// $.ajax({
	// 	url: 'http://baylabay.com/checkTicketSearch',
	// 	type: 'post',async: true,
	// 	crossDomain: true,
	// 	dataType: 'json',
	// 	data: {'roundtrip':1,'origin_airport_code':'HPH','destination_airport_code':'HAN','departure_date':'09/05/2020','return_date':'09/05/2020','adults':1,'children':0,'infants':0},
	// 	beforeSend: function() {
	...
	// 		if (json['success']) {
	// 			alert(json['success'])
	// 			var url = "layoutTicketSearch&sid=" + $.trim(json['success']);
	//
	// 			//window.location = url;
	// 		}
	// 	}
